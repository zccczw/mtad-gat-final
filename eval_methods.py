...
"""
这段代码是OmniAnomaly（一种异常检测算法）的实现，具体实现了POT方法，
即Peak Over Threshold，通过寻找在给定阈值下是否存在概率密度函数的峰值来检测异常。
在给定的分数数据上，先用SPOT（Subspace Pursuit Optimization Technique）方法进行初始化，
然后运行POT方法。具体的， pot_eval 函数根据训练集得到初始分数，然后在测试集上运行POT方法，
返回异常检测的性能指标，包括F1-score，精确率，召回率等等。 adjust_predicts 函数用来调整预测标签，
通过比较分数和给定阈值来对异常点进行标记。 
calc_point2point 函数计算预测标签和实际标签之间的F1-score。 bf_search 函数通过遍历不同的阈值，找到最佳的F1-score。
"""
